// ==UserScript==
// @name         New Userscript
// @namespace    http://tampermonkey.net/
// @version      2024-05-23
// @description  try to take over the world!
// @author       You
// @match        https://*/*
// @icon         https://koharuxenon.github.io/korusite/icon.png
// @grant        none
// ==/UserScript==

function getRandomNumber(min, max) {
    return Math.random() * (max - min) + min;
}

function computeVelocity(Velocity, Acceleration, Drag, Max, Elapsed) {
    if (Acceleration != 0) {
        Velocity += Acceleration * Elapsed;
    }
    else if (Drag != 0) {
        var drag = Drag * Elapsed;
        if (Velocity - drag > 0) {
            Velocity -= drag;
        }
        else if (Velocity + drag < 0) {
            Velocity += drag;
        }
        else {
            Velocity = 0;
        }
    }
    if ((Velocity != 0) && (Max != 0)) {
        if (Velocity > Max) {
            Velocity = Max;
        }
        else if (Velocity < -Max) {
            Velocity = -Max;
        }
    }
    return Velocity;
}

const standSprite = "url('data:image/png;base64,')";
const grabSprite = "url('data:image/png;base64,')";
const fallSprite = "url('data:image/png;base64,')";

let mouseX = 0;
let mouseY = 0;

const catW = 52;
const catH = 52;

let catX = getRandomNumber(0, window.innerWidth - catW);
let catY = 0;

let catPX = 0;
let catPY = 0;
let catPX2 = 0;
let catPY2 = 0;
let catPX3 = 0;
let catPY3 = 0;

let catVX = getRandomNumber(-500, 500);
let catVY = 0;

let gravity = 800

let airdrag = gravity / 2.75;
let bottomdrag = gravity * 2;

let xdrag = airdrag;
let accel = gravity;

let tFPS = 60;
let tFTIM = 1 / tFPS;

let grabSpritebed = false;

let grabSpriteoX = 0;
let grabSpriteoY = 0;

function gTime() {
    return Date.now() / 1000;
}

let startTime = gTime();
let prevtime = startTime - tFTIM;

function disableTextSelection() {
    document.body.style.userSelect = 'none';
    document.body.style.webkitUserSelect = 'none';
    document.body.style.mozUserSelect = 'none';
    document.body.style.msUserSelect = 'none';
}

function enableTextSelection() {
    document.body.style.userSelect = 'auto';
    document.body.style.webkitUserSelect = 'auto';
    document.body.style.mozUserSelect = 'auto';
    document.body.style.msUserSelect = 'auto';
}

window.addEventListener('mouseup', function (event) {
    enableTextSelection();
});

(function catty() {
    const catFren = document.createElement("div");
    catFren.style.backgroundImage = fallSprite;
    catFren.style.backgroundSize = "contain";
    catFren.style.imageRendering = "pixelated";
    catFren.style.width = `${catW}px`;
    catFren.style.height = `${catH}px`;
    catFren.style.position = "absolute";
    catFren.id = "catFren";

    catFren.style.left = catX + "px";
    catFren.style.top = catY + "px";
    catFren.style.zIndex = 99999999;

    document.body.appendChild(catFren);

    catFren.addEventListener('mousedown', function (event) {
        disableTextSelection();
        grabSpritebed = true;
        grabSpriteoX = catX - mouseX;
        grabSpriteoY = catY - mouseY;

        catFren.style.backgroundImage = grabSprite;

        catVX = 0;
        catVY = 0;
        accel = 0;
    });

    document.addEventListener("mousemove", function (event) {
        mouseX = event.clientX;
        mouseY = event.clientY;
    });

    window.addEventListener('mouseup', function (event) {
        if (grabSpritebed) {
            grabSpritebed = false;
            accel = gravity;
            xdrag = airdrag;

            catFren.style.backgroundImage = fallSprite;

            catVX = (catX - catPX2) * 7.5;
            catVY = (catY - catPY2) * 7.5;

            console.log(catVX, catVY);
        }
    });

    function frameUpdate() {
        startTime = gTime();
        let elapsed = startTime - prevtime;

        catPX3 = catPX2;
        catPY3 = catPY2;

        catPX2 = catPX;
        catPY2 = catPY;

        catPX = catX
        catPY = catY
        if (!grabSpritebed) {
            let velocityDelta = 0.5 * (computeVelocity(catVX, 0, xdrag, 0, elapsed) - catVX);
            catVX += velocityDelta;
            let delta = catVX * elapsed;
            catVX += velocityDelta;
            catX += delta;

            velocityDelta = 0.5 * (computeVelocity(catVY, accel, 0, 0, elapsed) - catVY);
            catVY += velocityDelta;
            delta = catVY * elapsed;
            catVY += velocityDelta;
            catY += delta;
        }
        else {
            catX = mouseX + grabSpriteoX;
            catY = mouseY + grabSpriteoY;
        }

        if (catX < 0) {
            catVX *= -0.5;
            catX = 0;
            //console.log("bounce left");
        }
        if (catX > window.innerWidth - catW) {
            catX = window.innerWidth - catW
            catVX *= -0.5;
            //console.log("bounce right");
        }
        if (catY < 0) {
            catVY = 0;
            catY = 0;
            //console.log("bounce top");
        }
        if (catY > window.innerHeight - catH) {
            if (catVY > 100) {
                catY = window.innerHeight - catH;
                catVY *= -0.4;
                //console.log("bounce bottom");
            }
            else {
                catVY = 0;
                accel = 0;
                catY = window.innerHeight - catH;
                xdrag = bottomdrag;
                if (!grabSpritebed)
                    catFren.style.backgroundImage = standSprite;
                //console.log("land");
            }
        }

        catFren.style.left = catX + "px";
        catFren.style.top = catY + "px";

        prevtime = gTime();
    }

    setInterval(frameUpdate, tFTIM * 1000);
})();
